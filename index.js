...
let changes = netProfit / finances.length
...
for (i = 1; i < finances.length; i++) {
    ...
    let monthOne = finances[0][1]

    if (difference > monthOne) {
        maxProfit = difference
        maxMonth = finances[i][0] + " $" + finances[i][1]
...
var minMonth
for (i = 1; i < finances.length; i++) {
    let pastMonth = finances[i - 1][1]
    let currentMonth = finances[i][1]
    let difference = currentMonth - pastMonth

    if (difference < maxLoss) {
...
console.log(`Greated decrease in profit (-$${Math.abs(maxLoss)}) is for ${minMonth}`)
